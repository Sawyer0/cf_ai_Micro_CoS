...
export class ThreadStateEntity {
	...
	appendToken(token: string): ThreadStateEntity {
		return new ThreadStateEntity(this.conversationId, true, this.currentStreamingMessage + token, null);
	}
	...
	clearError(): ThreadStateEntity {
		return new ThreadStateEntity(this.conversationId, this.isStreaming, this.currentStreamingMessage, null);
	}

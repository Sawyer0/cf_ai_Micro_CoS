...
import { ConversationEntity, type Conversation } from '@/domain/entities/Conversation';
...
export class ConversationRepository implements IConversationRepository {
	...
	async save(conversation: ConversationEntity): Promise<void> {
		...
		this.storage.setItem(STORAGE_KEY, JSON.stringify(all.map((c) => c.toJSON())));
	}

	async delete(id: string): Promise<void> {
		const all = await this.findAll();
		const filtered = all.filter((c) => c.id !== id);
		this.storage.setItem(STORAGE_KEY, JSON.stringify(filtered.map((c) => c.toJSON())));
	}
}

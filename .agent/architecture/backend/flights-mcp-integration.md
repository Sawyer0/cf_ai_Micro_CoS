...
│    - User adds event: "Paris trip, May 10–15"                   │
│    - Event metadata extracted: departure_city, destination,     │
│      travel_date, duration                                       │
└───────────────────┬──────────────────────────────────────────────┘
                    │
                    ▼
┌──────────────────────────────────────────────────────────────────┐
│ 2. Travel Event Hook Triggered                                   │
│    - Durable Object detects "travel event" pattern in calendar  │
│    - Extracts: origin city, destination, dates, preferences     │
│    - Schedules travel workflow (eventually, if not immediate)   │
└───────────────────┬──────────────────────────────────────────────┘
                    │
                    ▼
┌──────────────────────────────────────────────────────────────────┐
│ 3. Flights-MCP Tool Invocation                                   │
...
└───────────────────┬──────────────────────────────────────────────┘
                    ...
                    ▼
┌──────────────────────────────────────────────────────────────────┐
│ 4. LLM Ranking & Reasoning                                       │
│    - Pass user preferences + flight options to Llama 3.3        │
│    - LLM ranks flights based on:                                │
│      * User preferences (airlines, cabin, non-stop)            │
│      * Calendar context (meeting times, layover tolerance)     │
│      * Price vs. convenience tradeoffs                         │
│    - Output: Top 3 ranked options + rationale                  │
└───────────────────┬──────────────────────────────────────────────┘
                    │
                    ▼
┌──────────────────────────────────────────────────────────────────┐
│ 5. Persistent Storage & State Management                         │
│    - Store in Durable Object:                                   │
│      * flight_search_request (id, origin, dest, dates, status) │
│      * flight_search_results (tool_response, ranking, timestamp)
│      * user_travel_event (calendar_event_id, metadata)         │
│    - Mark request as completed/failed for idempotency          │
└───────────────────┬──────────────────────────────────────────────┘
                    │
                    ▼
┌──────────────────────────────────────────────────────────────────┐
│ 6. Frontend Notification & Interaction                           │
│    - Realtime push to frontend: "Flights found for Paris trip" │
│    - Display trip card with top 3 options                       │
...
**Responsibility:** Monitor calendar events for travel patterns, extract travel metadata.
...
**Logic:**
- Listen for calendar event updates (via webhook or polling).
...
interface TravelProfileDO {
  ...
  updatePreferences(prefs: Partial<UserTravelProfile["preferences"]>): Promise<void>;
...
interface ToolInvocationEvent {
  ...
  event_type: "travel_event_detected" | "flight_search_requested" | 
              "flight_search_completed" | "flight_ranking_started" | 
              "flight_ranking_completed" | "suggestions_published" | 
              "user_selection_made" | "workflow_error";
  timestamp: ISO8601;
  user_id: string;
  travel_event_id: string;
...
### Calendar → Travel Event Detection

```
User adds calendar event
        │
        ▼
Frontend POST /api/calendar/event
        │
        ▼
Worker receives event
        │
        ▼
TravelEventDetector DO:
  - Extract keywords (destination, date, type)
  - Call LLM to classify as "travel" (optional, for confidence)
  - Store TravelEvent
        │
        ▼
Emit hook: "travel_event_detected"
        │
        ▼
TravelWorkflow DO triggers
```

### Travel Workflow Orchestration

```
TravelWorkflow DO receives "travel_event_detected" hook
        │
        ▼
Check: is this a duplicate/already processed?
       (idempotency via travel_event_id)
        │
        ▼
YES → skip
NO  → continue
...
LLM returns [RankedFlight]
        │
        ▼
Store in TravelProfileDO.travel_history
        │
        ▼
...
Frontend receives Realtime notification
        │
        ▼
Display "Trip suggestions" card with top 3 flights
```

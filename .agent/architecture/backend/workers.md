...
There are **several kinds of Workers** (or Worker responsibilities) in the system:
...
- Forward events to:
  ...
  - **Python FastAPI backend** (core REST + chat API)
  - **Durable Objects** (state/memory and reducers)
  - **Workflows / background workers** (if background tasks are needed)
...
- Input validation (schema-based, e.g., Zod / Pydantic)
...
### **B. Calendar events sync for warm data (background)**

1. A scheduled **CalendarSyncWorker** (cron or webhook) keeps `CalendarEventStoreDO` **warm** by syncing calendar events for a user.
2. CalendarSyncWorker:

   - Fetches events from Google Calendar via MCP.
   - Normalizes them into the internal `CalendarEvent` model.
   - Sends batched updates to `CalendarEventStoreDO`.

3. **CalendarEventStoreDO**:

   - Upserts events into per-user calendar state.
   - Optionally helps drive detection and storage of `TravelEvent`s via downstream hooks, maintaining records in `TravelWorkflowDO` / `TravelProfileDO`.

4. **Important:** This background sync **does not**:

   - Call flights-MCP.
   - Run daily planner prompts.
   - Send user-visible messages by itself.

5. When the user later asks in chat (flow A) — e.g., "What's on my calendar next week?" or "Find flights for my Boston trip" — chat handlers and workflows read from `CalendarEventStoreDO` and `TravelEvent` records before orchestrating tools and LLMs.

Workflows can also trigger additional internal events (e.g., daily planner), but user-facing plans and suggestions still surface via chat-driven flows → DO → Realtime → Clients
...
- Workers + DOs + Realtime = Cloudflare-native edge platform in front of the Python FastAPI backend

...
export class DuffelFlightAdapter implements IFlightPort {
	...
	async searchFlights(request: FlightSearchRequest): Promise<FlightOption[]> {
		...
		try {
			...
			const response = await withRetry(
				() => this.client.post<DuffelOfferResponse>('/offer_requests', payload, correlationId),
				{
					maxAttempts: 3,
					initialDelayMs: 200,
					maxDelayMs: 2000,
	...
	async getFlightDetails(flightId: string): Promise<FlightOption | null> {
		...
		try {
			...
			const response = await this.client.get<{ data: DuffelFlightOffer }>(`/offers/${flightId}`, correlationId);

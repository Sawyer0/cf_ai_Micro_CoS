...
export class D1ChatAdapter implements IChatRepository {
	...
	async saveMessage(message: Message, conversationId: string): Promise<void> {
      ...
      `,
				...
				.bind(message.id.toString(), conversationId, message.role, message.content, message.timestamp.toISOString())
				.run();
		} catch (error) {
	...
	async getMessages(conversationId: string, limit: number, offset?: number): Promise<Message[]> {
      ...
      `,
				...
				.bind(conversationId, limit, offset || 0)
				.all();

			return results.results.map((row) =>
				Message.reconstitute(row.id as string, row.role as MessageRole, row.content as string, new Date(row.timestamp as string), {}),
			);
		} catch (error) {

...
export class ChatSessionDO extends DurableObject<WorkerEnv> {
    ...
    private async handleChat(request: Request): Promise<Response> {
        ...
        try {
            ...
            console.error('[ChatSession] Failed to retrieve long-term memory:', error);
        }

        // Trigger workflow or pre-fetch data based on intent
        try {
            if (intentResult.workflow === 'travel' && intentResult.entities.destination) {
                console.log('[ChatSession] Travel Intent Detected', intentResult.entities);
                ...
                console.log(`[ChatSession] API Key Status: ${apiKey ? 'Present' : 'Missing'}, Length: ${apiKey.length}, Value: ${apiKey === 'mock' ? 'mock' : '***'}`);
                ...
                if (flights.length > 0) {
                    ...
                    flights.slice(0, 3).forEach(f => {
                        ...
                        contextInjection += `- ${seg.airline} (${seg.flightNumber}): ${f.totalPrice} ${f.currency}, Departs ${seg.departureTime.toLocaleTimeString()}\n`;
                    });
                }
                ...
                console.log('[ChatSession] Triggering Task Workflow', intentResult.entities);
                ...
                console.log('[ChatSession] Triggering Planning Workflow', intentResult.entities);
            ...
            console.error('[ChatSession] Failed to trigger workflow/action:', error);
        }
        ...
        if (contextInjection) {
            console.log('[ChatSession] Injecting context into user message:', contextInjection.substring(0, 200) + '...');
            for (let i = messagesWithContext.length - 1; i >= 0; i--) {
                if (messagesWithContext[i]?.role === 'user') {
                    ...
                    console.log('[ChatSession] Context injected into message at index', i);
                    break;
                }
            }
        } else {
            console.log('[ChatSession] No context to inject');
        }
    ...
    private async handleWorkflowResult(request: Request): Promise<Response> {
        ...
        console.log('[ChatSession] Received workflow result:', message);

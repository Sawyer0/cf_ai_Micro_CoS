...
export class StorageManager {
    ...
    async logTurn(args: LogTurnArgs): Promise<void> {
        ...
        if (args.userMessage) {
            ...
            ).bind(crypto.randomUUID(), sessionId, args.conversationId, 'user', args.userMessage, args.correlationId, now).run();
        }

        // Log Assistant Message
        await this.db.prepare(
            'INSERT INTO chat_events (id, session_id, conversation_id, role, content, correlation_id, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)'
        ).bind(crypto.randomUUID(), sessionId, args.conversationId, 'assistant', args.assistantMessage, args.correlationId, now).run();

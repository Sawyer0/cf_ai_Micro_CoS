...
export async function handleChatConversationsRequest(
  ...
  const conversations = (results.results || []).map((row: any) => ({
    ...
    title: `Conversation ${((row.conversation_id as string) || (row.id as string)).slice(0, 8)}`, // TODO: Store actual title
...
export async function handleSaveConversationRequest(
  ...
  console.log('[SaveConversation] Received body:', JSON.stringify(body));

  if (!body || !body.id || !body.title) {
    console.log('[SaveConversation] Invalid body - id:', body?.id, 'title:', body?.title);
...
export async function handleDeleteConversationRequest(
  ...
  await db
    .prepare('DELETE FROM chat_events WHERE session_id = ?1')
    .bind(conversationId)
    .run();

  await db
    .prepare('DELETE FROM chat_sessions WHERE id = ?1 AND principal_id = ?2')
...
export async function handleChatHistoryRequest(
  ...
  console.log(`[ChatHistory] Fetching session ${conversationId} for principal ${principal.id}`);
  console.log(`[ChatHistory] Session found: ${!!session}`);

  if (!session) {
    console.warn(`[ChatHistory] Session not found: ${conversationId} (Principal: ${principal.id})`);
    return new Response('Conversation not found', { status: 404 });
  }

  const eventsResult = await db
    .prepare(
      'SELECT id, role, content, created_at FROM chat_events WHERE session_id = ?1 ORDER BY created_at ASC LIMIT ?2 OFFSET ?3'
    )

...
export class EpisodicMemory {
	...
	getRecentTrips(limit = 3): TravelHistoryEntry[] {
		return this.travelHistory.filter((t) => t.bookingStatus !== 'cancelled').slice(0, limit);
	}
	...
	toPromptContext(): string {
		...
		if (recentTrips.length > 0) {
			...
			recentTrips.forEach((trip) => {
				...
				const status = trip.bookingStatus === 'completed' ? '(completed)' : trip.bookingStatus === 'booked' ? '(booked)' : '(planned)';

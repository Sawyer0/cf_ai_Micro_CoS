...
export class Task {
	...
	static create(
		...
		const now = new Date();
		return new Task(crypto.randomUUID(), title, userId, 'pending', priority, metadata, now, now, description, dueDate);
	}
	...
	static reconstitute(
		...
		return new Task(id, title, userId, status, priority, metadata, createdAt, updatedAt, description, dueDate);
	}

...
export function buildFlightRankingPrompt(context: RankingContext): string {
    ...
    const flightList = flights.map((flight, idx) => {
...
`;
    ...
    const calendarInfo = calendarEvents.length > 0
...
export function parseFlightRanking(llmResponse: string | any[], flights: FlightOption[]): FlightOption[] {
    try {
        console.log('[DEBUG] parseFlightRanking called');
        console.log('[DEBUG] llmResponse type:', typeof llmResponse);
        console.log('[DEBUG] llmResponse value:', JSON.stringify(llmResponse));
        console.log('[DEBUG] Is array?', Array.isArray(llmResponse));
        ...
        if (Array.isArray(llmResponse)) {
            ...
            if (!match) {
                console.warn('Could not parse ranking from LLM response, returning original order');
                console.warn('LLM response:', llmResponse);
                return flights;
            }
            ranking = JSON.parse(match[0]);
        } else {
            console.warn('Unexpected LLM response type:', typeof llmResponse);
            return flights;
        }

        // Validate ranking
        if (!Array.isArray(ranking) || ranking.length !== flights.length) {
            console.warn(`Invalid ranking length: expected ${flights.length}, got ${ranking.length}`);
            return flights;
        }

        // Convert 1-based indices to 0-based and reorder flights
        return ranking
            .map(idx => flights[idx - 1])
            .filter(f => f !== undefined);
    } catch (error) {
        console.error('Error parsing flight ranking:', error);

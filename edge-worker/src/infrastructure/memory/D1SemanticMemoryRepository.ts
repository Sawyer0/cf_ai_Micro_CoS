...
export class D1SemanticMemoryRepository implements ISemanticMemoryRepository {
	...
	async getSemanticMemory(userId: string): Promise<SemanticMemory> {
		...
		// Fetch profile
		const profileResult = await this.db.prepare('SELECT * FROM user_profiles WHERE user_id = ?').bind(userId).first<any>();
		...
		const profile: UserProfile = {
			...
			notificationPreferences: profileResult.notification_preferences ? JSON.parse(profileResult.notification_preferences) : undefined,
	...
	async updateUserProfile(userId: string, updates: Partial<UserProfile>): Promise<void> {
		const allowedFields = ['home_airport', 'preferred_seat', 'budget_domestic_usd', 'budget_international_usd', 'notification_preferences'];
	...
	async addPreference(userId: string, pref: Omit<UserPreference, 'id' | 'userId' | 'createdAt' | 'updatedAt'>): Promise<void> {
      ...
      `,
			...
			.bind(id, userId, pref.category, pref.preferenceKey, pref.preferenceValue, pref.confidence || 1.0, pref.source || 'explicit')
			.run();
	}

...
export function estimateTokensFromMessages(messages: Array<{ role?: unknown; content?: unknown }>): number {
	let chars = 0;
...
export async function applyRateLimit(
	...
	const chatWindowStartSec = Math.floor(now / 1000 / chatWindowSeconds) * chatWindowSeconds;
	...
	if (tokensUsed + tokensEstimate > tokenLimit) {
		const resetDate = new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate() + 1);
	...
	try {
		await env.RATE_LIMIT_KV.put(chatKey, String(chatCount + 1), { expirationTtl: chatWindowSeconds + 5 });
	} catch {}
